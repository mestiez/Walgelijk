...
{
    /// <summary>
    /// Perlin noise utility struct
    /// </summary>
    public struct Perlin
    {
        /// <summary>
        /// Get fractal perlin noise between 0 to 1
        /// </summary>
        public static float FractalNoise(float x, float y, float z, int octaves, float persistence)
        {
            float total = 0;
            float frequency = 1;
            float amplitude = 1;
        ...
        /// <summary>
        /// Get perlin noise between 0 to 1
        /// </summary>
        public static float Get(float x, float y, float z)
        {
            ...
            float x1, x2, y1, y2;
            x1 = Utilities.Lerp(Grad(p[aa], xf, yf, zf),          // This is where the "magic" happens.  We calculate a new set of p[] values and use that to get
            ...
            x2 = Utilities.Lerp(Grad(p[ab], xf, yf - 1, zf),          // we interpolate between the y-values to get a z-value.
                        Grad(p[bb], xf - 1, yf - 1, zf),
                        u);                                     // When calculating the p[] values, remember that above, p[a+1] expands to p[xi]+yi+1 -- so you are
            y1 = Utilities.Lerp(x1, x2, v);                               // essentially adding 1 to yi.  Likewise, p[ab+1] expands to p[p[xi]+yi+1]+zi+1] -- so you are adding
                                                                // to zi.  The other 3 parameters are your possible return values (see grad()), which are actually
            x1 = Utilities.Lerp(Grad(p[aa + 1], xf, yf, zf - 1),      // the vectors from the edges of the unit cube to the point in the unit cube itself.
                        Grad(p[ba + 1], xf - 1, yf, zf - 1),
                        u);
            x2 = Utilities.Lerp(Grad(p[ab + 1], xf, yf - 1, zf - 1),
                          Grad(p[bb + 1], xf - 1, yf - 1, zf - 1),
                          u);
            y2 = Utilities.Lerp(x1, x2, v);

            return (Utilities.Lerp(y1, y2, w) + 1) / 2;                       // For convenience we bound it to 0 - 1 (theoretical min/max before is -1 - 1)
        }
