...
public readonly struct Dropdown<T> : IControl
{
    ...
    public Dropdown(IList<T> values, int selectedIndex)
    {
        Values = values;
    ...
    {
        var (instance, node) = Onion.Tree.Start(IdGen.Hash(nameof(Dropdown<ValueType>).GetHashCode(), identity, site), new Dropdown<ValueType>(values, selectedIndex));
        Onion.Tree.End();
        return instance.State;
    }
