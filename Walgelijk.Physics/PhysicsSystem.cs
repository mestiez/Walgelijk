...
namespace Walgelijk.Physics
{
    ...
    public class PhysicsSystem : Walgelijk.System
    {
        ...
        public override void Update()
        {
            ...
            //distributor.Rate = world.UpdatesPerSecond;
            //var iterationsThisFrame = distributor.CalculateCycleCount(Time.UpdateDeltaTime);
            ...
            //var back = DefaultLayers.Zero.WithOrder(-1);
            //foreach (var item in world.ChunkDictionary)
            //{
            //    var c = item.Value;
            //    var topleft = new Vector2(c.X, c.Y) * world.ChunkSize;
            //    DebugDraw.Rectangle(topleft, cSize, 0, c.IsEmpty ? Colors.Gray * .5f : Colors.Aqua, renderOrder: c.IsEmpty ? back : DefaultLayers.Zero);
            //}
        }
        ...
        public IEnumerable<Entity> GetBodiesNear(IntVector2 chunkPos, PhysicsWorldComponent world)
        {
            if (world.ChunkDictionary.TryGetValue(chunkPos, out var chunk))
            {
                //DebugDraw.Rectangle(new Vector2(pos.X * world.ChunkSize, pos.Y * world.ChunkSize), new Vector2(world.ChunkSize, world.ChunkSize), 0, Colors.Blue, renderOrder: DefaultLayers.Top);
        ...
        public int QueryPoint(Vector2 point, ref QueryResult[] results, uint filter = uint.MaxValue)
        {
            ...
            foreach (var entity in GetBodiesNear(chunkpos, world))
            {
                ...
                if (bodyComponent.Collider.IsPointInside(point))
                {
                    ...
                    {
                        Body = bodyComponent,
                        Collider = bodyComponent.Collider,
                        Entity = entity
                    };
                    i++;
                    if (i >= results.Length)
