...
public readonly struct EntryButton : IControl
{
    ...
    {
        var (instance, node) = Onion.Onion.Tree.Start(IdGen.Create(nameof(EntryButton).GetHashCode(), identity, site), new EntryButton());
        instance.RenderFocusBox = false;
        instance.Name = label;
        Onion.Onion.Tree.End();
